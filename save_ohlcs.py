...
class ohlcs:
    ...
    def get_instrument_token_for_niftybees(self):
        # returns: instrument token of the NIFTYBEES ETF
        # the instrument token is the equity instrument token for the underlying trading on nse:

        # filter instruments for nse and 'NIFTYBEES'
        potential_instruments = [instrument for instrument in self.instruments if instrument.get('exchange') == 'NSE' and instrument.get('tradingsymbol') == 'NIFTYBEES']
...
if __name__ == '__main__':
    ...
    niftybees_token = o.get_instrument_token_for_niftybees()
    start_date = pd.to_datetime('2010-01-01', format='%Y-%m-%d')
    end_date = pd.to_datetime('2023-03-24', format='%Y-%m-%d')
    ohlc = o.get_5min_ohlc_by_instrument_token(niftybees_token, start_date, end_date)
    # dump csv
    # ohlc.to_csv(os.path.join(o.data_dir, 'NIFTYBEES.csv'), index=False)
